...
// finalized time entry. Single atomic RPC (timer_finalize) that closes the open
// segment, honors the explicit start/end the user saw in the Save modal, applies the
// 1–59s→60s rounding, bootstraps any missing monday dimension rows, and flips
// timer_state to 'finalized' — no separate soft-reset/session cleanup needed.
//
...
// the RPC does not handle, so they stay here.
...
export async function POST(request: NextRequest) {
	try {
		...
		const { entryId, taskName, comment, boardId, boardName, itemId, itemName, parentItemId, parentItemName, roleId, duration, startTime, endTime } = body;
		...
		// Write-back to monday — fire-and-forget so it doesn't block the response.
