...
const IconSync = () => (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
            d="M2 8a6 6 0 0110.89-3.477M14 8a6 6 0 01-10.89 3.477"
            stroke="currentColor"
            strokeWidth="1.5"
            strokeLinecap="round"
            strokeLinejoin="round"
...
export default function BoardConfigPage() {
    ...
    const availableRolesForOverride = availableRoles.filter(
        (role) => !roleOverrides.some((o) => o.role_id === role.id),
    );
    ...
    return (
        <div id="admin-app">
            ...
            <Breadcrumbs mb="md">
                ...
                <Text>
                    {(boardConfig as any).monday_board?.name || (boardConfig as any).board_name || boardConfig.board_id}
                </Text>
            </Breadcrumbs>
                        ...
                        <div className="admin-section-header">
                            <div>
                                ...
                                <p className="admin-section-description">
                                    Configure which monday.com columns receive synced time data.
                                </p>
                            </div>
                        ...
                        {columns.length === 0 ? (
                            <div className="empty-state">
                                ...
                                <p className="empty-state-description">
                                    Add a column mapping to sync time data to a monday.com column.
                                </p>
                                ...
                                <Table.Tbody>
                                    {columns.map((column) => (
                                        <Table.Tr key={column.id}>
                                            ...
                                            <Table.Td>
                                                <Badge color={column.sync_enabled ? "green" : "gray"}>
                                                    {column.sync_enabled ? "Enabled" : "Disabled"}
                                                </Badge>
                                            </Table.Td>
                                            <Table.Td>
                                                <Group gap="xs">
                    ...
                    <div className="admin-section">
                        <div className="admin-section-header">
                            <div>
                                <h2>Groups</h2>
                                <p className="admin-section-description">
                                    Control which groups are synced for the task selector. Disabled groups will not
                                    appear in the task dropdown.
                                </p>
                            </div>
                        </div>
                        ...
                        {groupsLoading ? (
                            ...
                            <div className="empty-state">
                                ...
                                <p className="empty-state-description">
                                    This board has no groups, or they haven't been loaded yet. Groups will be fetched
                                    from monday.com when you open this tab.
                                </p>
                            </div>
                        ) : (
                            <Table>
                                ...
                                <Table.Tbody>
                                    {groups.map((group) => (
                                        <Table.Tr key={group.id}>
                                            ...
                                            <Table.Td>
                                                <Badge color={group.sync_enabled ? "green" : "gray"}>
                                                    {group.sync_enabled ? "Synced" : "Not Synced"}
                                                </Badge>
                                            </Table.Td>
                                            <Table.Td>
                                                <Switch
                                                    label={group.sync_enabled ? "Enabled" : "Disabled"}
                                                    checked={group.sync_enabled}
                                                    onChange={() => handleToggleGroupSync(group.id, group.sync_enabled)}
                                                />
                                            </Table.Td>
                                        </Table.Tr>
                                    ))}
                    ...
                    <div className="admin-section">
                        <div className="admin-section-header">
                            <div>
                                ...
                                <p className="admin-section-description">
                                    Override default hourly rates for specific roles on this board.
                                </p>
                            </div>
                            <Button
                                leftSection={<IconPlus />}
                                onClick={() => handleOpenOverrideModal()}
                                disabled={availableRolesForOverride.length === 0}
                            >
                                Add Rate Override
                            </Button>
                        </div>
                        ...
                        {roleOverrides.length === 0 ? (
                            <div className="empty-state">
                                ...
                                <p className="empty-state-description">
                                    All roles use their default hourly rates. Add an override to customize rates for
                                    this board.
                                </p>
                                {availableRolesForOverride.length > 0 && (
                                    <Button onClick={() => handleOpenOverrideModal()}>Add Rate Override</Button>
                                )}
                            </div>
                        ) : (
                            <Table>
                                ...
                                <Table.Tbody>
                                    {roleOverrides.map((override) => (
                                        <Table.Tr key={override.id}>
                                                ...
                                                <Flex align="center" gap="xs">
                                                    <div
                                                        style={{
                                                            ...
                                                            height: 12,
                                                            borderRadius: "50%",
                                                            backgroundColor: override.role?.color_hex || "#0073ea",
                                                        }}
                                                    />
                                            ...
                                            <Table.Td>
                                                <Text c="dimmed">
                                                    €{override.role?.hourly_rate?.toFixed(2) || "0.00"}
                                                </Text>
                                            </Table.Td>
                                            <Table.Td>
                                            ...
                                            <Table.Td>
                                                <Badge color={override.is_enabled ? "green" : "gray"}>
                                                    {override.is_enabled ? "Active" : "Inactive"}
                                                </Badge>
                                            </Table.Td>
                                            <Table.Td>
                                                <Group gap="xs">
                    ...
                    <div className="admin-section">
                        <div className="admin-section-header">
                            <div>
                                ...
                                <p className="admin-section-description">
                                    Manually sync time data to monday.com columns or view sync statistics.
                                </p>
                            </div>
                        </div>
                        ...
                        <Stack gap="lg">
                            ...
                            <Card withBorder padding="lg">
                                ...
                                <Text size="sm" c="dimmed" mb="md">
                                    Sync time data for all items on this board that have time entries. This will update
                                    all configured columns based on your column mappings.
                                </Text>
                                ...
                                {syncProgress > 0 && (
                                    <Progress
                                        ...
                                        mb="md"
                                        animated={syncing}
                                        color={syncProgress === 100 ? "green" : "blue"}
                                    />
                                )}

                                <Button
                                    leftSection={<IconSync />}
                                    onClick={handleBulkSync}
                                    loading={syncing}
                                    disabled={!boardConfig.sync_enabled || columns.length === 0}
                                >
                                ...
                                {!boardConfig.sync_enabled && (
                                    <Text size="sm" c="red" mt="sm">
                                        Sync is disabled for this board. Enable it in the board settings to use this
                                        feature.
                                    </Text>
                                )}
            ...
            >
                <Stack gap="md">
                    ...
                    {editingColumn && (
                        <TextInput
                            label="Column"
                            value={`${columnForm.column_name} (${columnForm.column_type})`}
                            disabled
                        />
                    )}
                    ...
                    <Switch
                        label="Enable sync"
                        description="Toggle sync for this column mapping"
                        checked={columnForm.sync_enabled}
                        onChange={(e) => setColumnForm({ ...columnForm, sync_enabled: e.currentTarget.checked })}
                    />
            ...
            <Modal
                ...
                title={editingOverride ? "Edit Rate Override" : "Add Rate Override"}
                size="md"
            >
                <Stack gap="md">
                    {!editingOverride && (
                    ...
                    {editingOverride && (
                        <TextInput label="Role" value={editingOverride.role?.name || "Unknown Role"} disabled />
                    )}

                    <NumberInput
                        label="Override Hourly Rate (€)"
                        description="This rate will be used instead of the default for this board"
                        placeholder="0.00"
                    ...
                    <Switch
                        label="Active"
                        description="Toggle whether this override is applied"
                        checked={overrideForm.is_enabled}
                        onChange={(e) => setOverrideForm({ ...overrideForm, is_enabled: e.currentTarget.checked })}
                    />
