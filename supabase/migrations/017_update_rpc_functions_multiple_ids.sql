-- Migration: 017_update_rpc_functions_multiple_ids
-- Updates RPC functions to support multiple item IDs for aggregation (e.g., Budget board roll-up)

-- Drop old versions to avoid overloading (PostgreSQL creates new functions if parameter types change)
...
-- Update get_item_total_time to support multiple IDs
...
-- Update get_item_time_by_role to support multiple IDs
...
-- Update calculate_remaining_budget to support multiple IDs
...
BEGIN
    ...
    -- Includes both the parent items and their sub-items
    SELECT COALESCE(SUM(
