...
| [timer.types.ts](timer.types.ts) | `@/types/timer.types` | Client-side timer domain: state/store/component-prop/API-response types, plus the only runtime code in this dir (`toTimerStatus`/`fromTimerStatus`). |
| [IconProps.ts](IconProps.ts) | `@/types/IconProps` | `IconProps` / `IconComponentProps` for `components/icons/`. Mostly superseded by the local `types.ts` files under `components/ui/icons/` and `components/ui/buttons/`. |
...
- `database.types.ts` layers convenience aliases over that (e.g. `Role`, `BoardConfig`, `ColumnSyncConfig`, `MondayWebhook` and their `*Insert`/`*Update` variants), plus types the generator can't express:
  - **RPC result types** — `GetItemTimeByRoleResult`, `CalculateRemainingBudgetResult`, `GetCurrentElapsedTimeResult`, `GetTimerSessionWithElapsedResult`, `FinalizeSegmentResult`. The generated `Functions` entries return `Json` for the timer RPCs; these interfaces give that `Json` a shape — cast RPC results to them.
...
- **`TimerSession` name collision.** It's exported from *both* [timer.types.ts](timer.types.ts) (client shape, nests an optional `time_entry`) and `@/types/database` (the generated `timer_session` Row). Different shapes — import from the path matching which one you mean.
