			...
			<div className={classes.toolbar}>
				...
				<Collapse expanded={showFilters} className={classes.filtersCollapse} transitionDuration={200} keepMounted>
					<div className={classes.filtersRow}>
						<Select placeholder="Rolle" data={filterOptions.roles.map((r) => ({ value: r.id, label: r.name }))} value={filters.roleId} onChange={(value) => setFilter({ roleId: value })} clearable />
						<Select placeholder="Board" data={filterOptions.boards.map((b) => ({ value: b.id, label: b.name }))} value={filters.boardId} onChange={(value) => setFilter({ boardId: value })} clearable />
						<Select placeholder="Status" data={filterOptions.timerState.map((t) => ({ value: t.id, label: t.name }))} value={filters.timerState} onChange={(value) => setFilter({ timerState: value })} clearable />
						<DatePicker
							type="range"
