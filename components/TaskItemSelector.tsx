...
export default function TaskItemSelector({
    onSelectionChange,
    onResetRef,
    initialValues,
    subItemsOnly,
}: TaskItemSelectorProps) {
    ...
    const handleTaskChange = useCallback(
        (value: string | null, option: ComboboxItem) => {
            ...
            if (value) {
                const allItems = tasks.flatMap((group) => group.items);
                fullOption =
                    (allItems.find((item) => (item as DropdownOption).value === value) as DropdownOption) || null;
            }
            ...
            {loadingBoards ? (
                ...
                <div>
                    ...
                    <Select
                        ...
                        data={boards}
                        value={selectedBoard?.value || null}
                        onChange={handleBoardChange}
                        clearable
                        searchable
                        disabled={loadingBoards}
                        nothingFoundMessage="Keine Boards verfügbar"
                    ...
                    {selectedBoard && (
                        <Tooltip label="Aufgabenliste aktualisieren" position="left">
                            <IconButton
                                ...
                                disabled={isRefreshing || isFetchingTasks}
                ...
                <Select
                    ...
                    nothingFoundMessage={
                        !selectedBoard
                            ...
                            : isTaskDropdownLoading
                              ? "Lade Aufgaben..."
                              : "Keine Aufgaben gefunden"
                    }
                    // Show loading indicator for initial load or background refetch
                    rightSection={
                        isTaskDropdownLoading || (isFetchingTasks && hasTaskData) ? <Loader size={14} /> : undefined
                    }
                    classNames={{ option: styles.selectOption }}
                />
            </div>
